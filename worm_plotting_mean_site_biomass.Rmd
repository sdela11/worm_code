...
keeps <- keeps <- c("site", "rep", "treatment", "forest_type", "inv_lvl", "biomass_m2")
...
worm_site_summary$string <- substring(worm_site_summary['site'], 1,2)

worm_site_summary[is.na(worm_site_summary[,1])] <- substring(worm_site_summary["site"], 1,2)

str_replace(worm_site_summary$treatment, "NA", substring(worm_site_summary['site'], 1, 2))

sub.test <- substr(worm_site_summary["site"], 1,3)
  
worm_site_summary <- worm_site_summary %>% 
  mutate(inv_lvl = case_when(
    is.na(site) & grepl("2", treatment) ~ "2",
    is.na (site) & grepl("5", treatment) ~ "5"
  ), .after = "site_name") 
