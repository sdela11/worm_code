...
wormplot.FUN <- function(data, plot.name, title){ #plot construction, variables and coordinates
  ...
  scale_fill_grey(labels = c("Endogeic", "Epi-anecic", "Epi-endogeic", "Epi-endogeic/anecic", "Epigeic")) + #sets color palette
  geom_col(position = position_dodge(), width = .7, colour = "black", linewidth = 1) +
  #geom_errorbar(aes(ymin = mean-SE, ymax = mean+SE),
  #              linewidth = 1,
  #              width = .2,
  #              position = position_dodge(.9)) +
    scale_y_continuous(limits = c(0,5), expand = c(0,0)) #set scale so that both plots will have the same y-axis.
